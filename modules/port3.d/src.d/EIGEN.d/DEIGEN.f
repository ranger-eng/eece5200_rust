...
C/6S
     ...
     1   29HDEIGEN - N IS GREATER THAN NM,29,1,2)
C/7S
C     IF (N .GT. NM) CALL SETERR(
C    1   'DEIGEN - N IS GREATER THAN NM',29,1,2)
C/
C
...
C/6S
  10  CALL SETERR(
     1   34HDEIGEN - FAILED ON THAT EIGENVALUE,34,IERR,1)
C/7S
C 10  CALL SETERR(
C    1   'DEIGEN - FAILED ON THAT EIGENVALUE',34,IERR,1)
C/
C
      CALL ISTKRL(1)
      RETURN
